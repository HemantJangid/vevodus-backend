...
exports.getShopDetails = (callback, userID) =>{
	...
	var query =  'select shop.SHOP_ID as shopId, shop.NAME as shopName, shop.LAT as lat, shop.lang as lang, shop.ADRESS as address,  shop.BROADER_CATEGORY as broaderCategory, shop.GST as gst, shop.VERIFIED as verified, shop.PHOTO_LINK as photoLink from VD_SHOP as shop inner join VD_SHOP_OWNER as sowner on  sowner.USER_ID =' + userID;
...
exports.getAllShops = (callback) => {
	var query = " select  NAME as name, LAT as lat, LANG as lang, ADRESS as address,  BROADER_CATEGORY as broaderCategory, GST as gst, VERIFIED as verified, PHOTO_LINK as photoLink from VD_SHOP";
	DBUtil.query(query, (err, recordsets) => {
		if(err == null) {
				callback(recordsets['recordset']);
		}
...
exports.addNewShop = (shopAttrs) =>{
	var query = "insert into VD_SHOP(NAME, LAT, LANG, ADRESS, BROADER_CATEGORY, GST, VERIFIED)"
	query += " values('" + shopAttrs['shopName'] + "', '" + shopAttrs['lat'] +"','" + shopAttrs['lang'] +"', '" + shopAttrs['address'] +"','" + shopAttrs['broaderCategory'] + "','" + shopAttrs['GST']+"',0); SELECT SCOPE_IDENTITY() as shopid;";
	return query;
}
